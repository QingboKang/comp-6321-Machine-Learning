% This is the solution for the NO.1 problem part a of Assignment 2
...
count = 1;
for lamda = 0:1:100
    % obtain weight vector perform L2 regularization
    weight(count, :) = (Xtrain'*Xtrain + lamda.*eye(num_features))^-1 * Xtrain' * Ytrain;
...
title('the RMSE on the training and test set with different \lambda')
...
figure;
plot( [0: 1: 100], weight(:, 1), [0: 1: 100], weight(:, 2), ...
     [0: 1: 100], weight(:, 3), [0: 1: 100], weight(:, 4));
grid on;
title('Weights and \lambda');
